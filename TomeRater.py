class User(object):
    ...
    def __repr__(self):
        return "Reviewer: {name}, email: {email}, read: {read}".format(name=self.name, email=self.email, read=len(self.books))
...
class Book(object):
    ...
    def add_rating(self, rating):
        if rating is not None and rating >= 0 and rating <= 4: # only way to not get NoneType Error? revisit to shorten.
...
class Fiction(Book): # SUBCLASS OF BOOK
    def __init__(self, title, author, isbn):
        super().__init__(title, isbn)
        self.author = author

    def get_author(self):
        return self.author # returns string.

    def __repr__(self):
        return "{title} by {author}".format(title=self.title, author=self.author)

class Non_Fiction(Book): # SUBCLASS OF BOOK
    def __init__(self, title, subject, level, isbn):  # level is a string like 'advanced'
        super().__init__(title, isbn)
...
class TomeRater(object):
    ...
    def add_user(self, name, email, user_books=None):
        if email not in self.users: #.items() removed.
            user = User(name, email)
            self.users[email] = user
            if user_books: # if there's anything in book field, avoid TypeError w/ None
    ...
    def highest_rated_book(self):
        ...
        for book in self.books.keys():
            book_avg_num = book.get_average_rating() # 'user' class method.
    ...
    def get_most_read_book(self): # forgot method, added.
        book_count = 0
        most_read = None
        for book in self.books:
